...
# Note: stochastic merge with tie-break noise has been removed.
...
def build_real_adj_from_heatmap(adj_mat, np_points, edge_index_np=None, sparse_graph=False):
  ...
  Returns (real_adj_mat, merge_iterations). Always deterministic (no tie-break noise).
  """
  if not sparse_graph:
  ...
  # Deterministic Cython merge only (tie-break noise removed)
  real_adj_mat, merge_iterations = cython_merge(np_points, sym_adj)
  return real_adj_mat, merge_iterations


def decode_tour_from_real_adj(real_adj_mat, start_node=0):
  """Decode a single tour from a 0/1 adjacency matrix starting at `start_node`.

  Deterministic tie-break: when two neighbors are available, pick the max index.
  """
...
# Note: multi-start decoding and random tie-break have been removed to simplify decoding.
